  ...
  if not test_flag:
    completion = client.chat.completions.create(
      ...
      messages=[
        ...
        and craft a plan that makes the topic easy to learn and apply. Answer consistently in JSON format that can be easily parsed with python json.loads, using exactly the following field names: \
          {weeks: {week_index, topic, topic_overview, assignments (title, estimated_time, youtube_queries, learning_goals), youtube_queries}}. Topic_overview is a 3 sentence summary of the week's topic. week.youtube_queries give 3 youtube search queries for providing an overview of the week's topic. Provide at least 3 assignments for each week. For each assignment, give 3 learning goals and 3 youtube search queries for finding relevant videos that provide effective practice of skills.\
